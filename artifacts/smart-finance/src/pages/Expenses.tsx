  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Outflows & Burn Rate</h1>
          <p className="text-muted-foreground mt-1">Track monthly expenses and living costs.</p>
        </div>
        <Button onClick={() => handleOpenDialog()} className="hover-elevate font-bold uppercase tracking-wider">
          <Plus className="mr-2 h-4 w-4" /> Log Outflow
        </Button>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editingExpense ? 'Update Outflow' : 'Log Outflow'}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input {...field} placeholder="e.g. Rent, Groceries, Netflix" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm bg-muted/20">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base font-bold text-destructive">Essential Expense</FormLabel>
                      <div className="text-sm text-muted-foreground">Required for basic survival (housing, food, etc). Eliminable if false.</div>
                    </div>
                    <FormControl>
              ...
              <div className="flex justify-end pt-4">
                <Button type="submit" disabled={createExpense.isPending || updateExpense.isPending}>
                  {editingExpense ? 'Update' : 'Save'} Outflow
                </Button>
              </div>
            </form>
          </Form>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="border-2 border-primary/20 md:col-span-2">
            <CardHeader className="pb-2">
              ...
              <CardDescription>Monthly capital outflow</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-4xl font-bold mb-6">{formatCurrency(burnRate.totalMonthly)}</div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-md bg-destructive/10 border border-destructive/20">
                  <div className="text-xs font-bold text-destructive uppercase tracking-wider mb-1">Essential Load</div>
                  <div className="text-2xl font-mono">{formatCurrency(burnRate.essentialTotal)}</div>
                  <div className="text-xs text-muted-foreground mt-1">{(burnRate.essentialTotal / burnRate.totalMonthly * 100 || 0).toFixed(0)}% of total</div>
                </div>
                <div className="p-4 rounded-md bg-muted border">
                  <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Variable Load</div>
                  <div className="text-2xl font-mono">{formatCurrency(burnRate.variableTotal)}</div>
                  <div className="text-xs text-muted-foreground mt-1">{(burnRate.variableTotal / burnRate.totalMonthly * 100 || 0).toFixed(0)}% of total</div>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Line-by-line outflow records</CardDescription>
          </div>
                  ...
                  {filteredExpenses.map((expense) => {
                    ...
                    return (
                      <TableRow key={expense.id}>
                        ...
                        <TableCell>
                          {expense.isEssential ? (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-destructive/10 text-destructive uppercase">Essential</span>
                          ) : (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-muted text-muted-foreground uppercase">Variable</span>
                          )}
                        </TableCell>
