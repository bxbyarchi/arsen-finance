  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Расходы и норма затрат</h1>
          <p className="text-muted-foreground mt-1">Учёт ежемесячных расходов и прожиточных затрат.</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editingExpense ? 'Обновить расход' : 'Новый расход'}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input {...field} placeholder="напр. Аренда, Продукты, Подписки" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm bg-muted/20">
                    <div className="space-y-0.5">
                      ...
                      <div className="text-sm text-muted-foreground">Необходимо для базового выживания (жильё, еда и т.д.).</div>
                    </div>
                    <FormControl>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="border-2 border-primary/20 md:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle>Общая норма затрат</CardTitle>
              <CardDescription>Ежемесячный отток капитала</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-4xl font-bold mb-6">{formatCurrency(burnRate.totalMonthly)}</div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-md bg-destructive/10 border border-destructive/20">
                  <div className="text-xs font-bold text-destructive uppercase tracking-wider mb-1">Обязательная нагрузка</div>
                  <div className="text-2xl font-mono">{formatCurrency(burnRate.essentialTotal)}</div>
                  <div className="text-xs text-muted-foreground mt-1">{(burnRate.essentialTotal / burnRate.totalMonthly * 100 || 0).toFixed(0)}% от общего</div>
                </div>
                <div className="p-4 rounded-md bg-muted border">
                  <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Переменная нагрузка</div>
                  <div className="text-2xl font-mono">{formatCurrency(burnRate.variableTotal)}</div>
                  <div className="text-xs text-muted-foreground mt-1">{(burnRate.variableTotal / burnRate.totalMonthly * 100 || 0).toFixed(0)}% от общего</div>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-0">
              <CardTitle className="text-sm">Распределение</CardTitle>
            </CardHeader>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Реестр расходов</CardTitle>
            <CardDescription>Детализированный учёт расходов</CardDescription>
          </div>
                  ...
                  {filteredExpenses.map((expense) => {
                    ...
                    return (
                      <TableRow key={expense.id}>
                        ...
                        <TableCell>
                          {expense.isEssential ? (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-destructive/10 text-destructive uppercase">Обязательный</span>
                          ) : (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-muted text-muted-foreground uppercase">Переменный</span>
                          )}
                        </TableCell>
                        <TableCell className="text-right font-mono font-medium">{formatCurrency(expense.amount)}</TableCell>
                        <TableCell>
